...
it.effect("always grants pull-requests and gates browser and device access independently", () =>
  Effect.gen(function* () {
    ...
    expect(yield* capabilitiesOf(withPreview)).toEqual(["preview", "pull-requests", "session"]);
    expect(yield* capabilitiesOf(withoutPreview)).toEqual(["pull-requests", "session"]);
    expect(yield* capabilitiesOf(withDevice)).toEqual(["device", "pull-requests", "session"]);
  }),
);
