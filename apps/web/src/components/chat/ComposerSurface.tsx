...
function Shell({
  ...
  return (
    <div
      ...
      className={cn(
        "@container/composer-surface group/composer-surface relative isolate mx-auto w-full max-w-[var(--provider-chrome-column,48rem)]",
        ...
        "before:pointer-events-none before:absolute before:inset-0 before:z-0 before:rounded-[var(--provider-chrome-radius,22px)] before:bg-[color-mix(in_srgb,var(--chat-composer-glass-surface)_var(--glass-opacity),transparent)] before:backdrop-blur-(--glass-blur) before:backdrop-saturate-(--glass-saturation)",
...
function Main({ className, ...props }: ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "group relative z-10 rounded-[var(--provider-chrome-radius,22px)] p-px transition-colors duration-[var(--provider-dur-base,200ms)]",
        outlineClasses,
        contextSeamClasses,
