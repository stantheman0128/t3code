...
`developer.togglePerformanceBar` shows or hides a developer footer with renderer
FPS and related metrics. It defaults to `mod+alt+shift+f` and stays off until
you turn it on.

`rightPanel.toggleMaximized` maximizes or restores the open right panel. It has
no default shortcut.
